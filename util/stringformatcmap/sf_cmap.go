...
// cmd: genx -pkg github.com/OneOfOne/cmap -v -t KT=string,VT=*util.FormatResults -name sf_cmap -o stringformatcmap/sf_cmap.go
// +build !genx
...
func (cm *CMap) iterContext(ctx context.Context, ch chan<- *KV, locked bool) {
	fn := func(k string, v *util.FormatResults) bool {
		select {
		case <-ctx.Done():
			return false
		case ch <- &KV{k, v}:
...
type LMap struct {
	m map[string]*util.FormatResults
	l *sync.RWMutex
}
...
func NewLMapSize(cap int) *LMap {
	return &LMap{
		m: make(map[string]*util.FormatResults, cap),
		l: new(sync.RWMutex),
	}
}
